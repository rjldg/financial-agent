...
Note for the implementer: `pytest.ini` sets `asyncio_mode = auto`, so the `async def`
tests above need no decorator. `"mystery purchase 999"` is deliberately chosen to miss
the lexicon so it reaches Tier 1.

- [ ] **Step 2: Run test to verify it fails**

Run: `python -m pytest tests/test_message_routing.py -q`
Expected: FAIL — `ImportError: cannot import name 'RouterParseError'` is already resolved by
Task 3, so the failures are `_handle_log_direct` still existing and
`AttributeError: module 'app.bot.handlers.transactions' has no attribute '_UNREADABLE_REPLY'`.
